...
ENTRY_EMA_SPREAD_ATR_MIN = 0.10
HTF_EMA_SPREAD_PCT_MIN = 0.0010
REQUIRE_HTF_SLOPE_OK = True
...
# =========================================================
def main():
    print(f"[{now_kst_str()}] === Bybit Auto Bot A-only balanced version start ===")
    bootstrap_state()

    send_telegram_message(
        f"[봇 시작]\n상태: A전략 밸런스 버전 시작\n심볼: {', '.join(SYMBOLS)}\n시간: {now_kst_str()}",
        key="bot_start_a_only_balanced",
        force=True
    )
