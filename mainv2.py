...
# 눌림 / B급
PULLBACK_ATR_MULTIPLIER = 1.8
B_PULLBACK_VALID_BARS = 8

# 매물대 필터
SR_LOOKBACK_BARS = 50
SR_BLOCK_ATR_MULTIPLIER = 0.4

# 최소 기대 수익 공간 필터
MIN_TARGET_ROOM_ATR = 1.2
...
def adjust_qty_to_step(qty: float, qty_step: float) -> float:
    decimals = decimals_from_step(qty_step)
    adjusted = round_down_by_step(qty, qty_step)
    return float(f"{adjusted:.{decimals}f}")


def should_log_error(symbol: str, func_name: str) -> bool:
    key = f"{symbol}:{func_name}"
    now_ts = time.time()
    last_ts = last_error_sent.get(key, 0)
    if now_ts - last_ts >= ERROR_COOLDOWN_SEC:
        last_error_sent[key] = now_ts
        return True
    return False
...
def log_error(symbol: str, func_name: str, error_text: str, order_failed=False, api_message=""):
    if not should_log_error(symbol, func_name):
        return
...
def reset_position_state(symbol: str):
    ...
    state[symbol]["entry_volume_ratio"] = None
...
def acquire_lock():
    ...
    try:
        ...
        print(f"[{now_kst_str()}] 이미 실행 중입니다. 중복 실행 차단.")
        raise SystemExit(1)


def handle_exit_signal(signum, frame):
    print(f"[{now_kst_str()}] 종료 신호 수신: {signum}")
    release_lock()
    raise SystemExit(0)
...
# =========================================================
def get_wallet_info():
    result = api_call(session.get_wallet_balance, accountType="UNIFIED")
...
def get_ticker_price(symbol: str) -> float:
    result = api_call(session.get_tickers, category=CATEGORY, symbol=symbol)
    return safe_float(result["result"]["list"][0]["lastPrice"])


def get_instrument_info(symbol: str):
    result = api_call(session.get_instruments_info, category=CATEGORY, symbol=symbol)
...
def get_position_from_exchange(symbol: str):
    result = api_call(session.get_positions, category=CATEGORY, symbol=symbol)
...
def get_entry_tf_data(symbol: str):
    ...
    add_volume_ma(df, VOLUME_MA_PERIOD)

    closed = df.iloc[:-1].copy().reset_index(drop=True)
    if len(closed) < max(50, SR_LOOKBACK_BARS + 5):
        return None
    ...
    sr_window = closed.iloc[-(SR_LOOKBACK_BARS + 1):-1].copy()
...
def update_pullback_state(symbol: str, signal: dict):
    ...
    if (
        ...
        and abs(current_price - ema21) <= atr * PULLBACK_ATR_MULTIPLIER
        and current_price <= signal["prev1_high"]
    ):
        prepare_pullback_state(
            symbol=symbol,
            direction="LONG",
            current_bar_time=bar_time,
            ref_high=safe_float(signal["prev1_high"]),
            ref_low=safe_float(signal["prev1_low"]),
        )
    ...
    if (
        ...
        and signal["ema9"] < signal["ema21"]
        and abs(current_price - ema21) <= atr * PULLBACK_ATR_MULTIPLIER
        and current_price >= signal["prev1_low"]
    ):
        prepare_pullback_state(
            symbol=symbol,
            direction="SHORT",
...
def get_entry_signal(symbol: str):
    ...
    long_pullback = current_price <= ema21 + (atr * 0.2) if atr > 0 else False
    long_recover = current_price > ema9
    long_A = long_trend and long_pullback and long_recover

    short_trend = ema9 < ema21
    short_pullback = current_price >= ema21 - (atr * 0.2) if atr > 0 else False
    short_recover = current_price < ema9
    short_A = short_trend and short_pullback and short_recover

    bearish_cross = (safe_float(p1["ema9"]) >= safe_float(p1["ema21"])) and (ema9 < ema21)
    bullish_cross = (safe_float(p1["ema9"]) <= safe_float(p1["ema21"])) and (ema9 > ema21)

    adx_ok = adx >= ADX_MIN
    adx_strong = adx >= ADX_STRONG
    adx_improving = adx_is_improving(p2, p1, c)

    recent_resistance = safe_float(sr_window["high"].max()) if len(sr_window) > 0 else 0.0
    recent_support = safe_float(sr_window["low"].min()) if len(sr_window) > 0 else 0.0

    upside_room = recent_resistance - current_price if recent_resistance > 0 else 999999.0
    downside_room = current_price - recent_support if recent_support > 0 else 999999.0

    sr_ok_long = True if atr <= 0 else upside_room >= (atr * SR_BLOCK_ATR_MULTIPLIER)
    sr_ok_short = True if atr <= 0 else downside_room >= (atr * SR_BLOCK_ATR_MULTIPLIER)

    target_room_ok_long = True if atr <= 0 else upside_room >= (atr * MIN_TARGET_ROOM_ATR)
    target_room_ok_short = True if atr <= 0 else downside_room >= (atr * MIN_TARGET_ROOM_ATR)

    volume_ratio = (volume / vol_ma) if vol_ma and vol_ma > 0 else 0.0
    ...
    signal = {
        ...
        "sr_ok_long": sr_ok_long,
        "sr_ok_short": sr_ok_short,
        "target_room_ok_long": target_room_ok_long,
        "target_room_ok_short": target_room_ok_short,
    ...
    if signal["long_A"]:
        print(f"[{now_kst_str()}] {symbol} A_SIGNAL_LONG")

    if signal["short_A"]:
        print(f"[{now_kst_str()}] {symbol} A_SIGNAL_SHORT")
...
def compute_position_qty(symbol: str, equity: float, entry_price: float, atr: float):
    ...
    final_qty = adjust_qty_to_step(final_qty, info["qty_step"])

    if symbol == "XRPUSDT":
        final_qty = int(final_qty)

    if final_qty < info["min_order_qty"]:
...
def open_position(symbol: str, direction: str, entry_type: str, signal: dict, wallet_info: dict):
    ...
    state[symbol]["entry_volume_ratio"] = volume_ratio
    state[symbol]["trail_active"] = False
    state[symbol]["trail_price"] = None
...
def close_position(symbol: str, reason: str, exit_bar_time=None):
    ...
    log_trade(
        ...
        entry_volume_ratio=state[symbol]["entry_volume_ratio"] or 0.0,
    )
...
def process_symbol(symbol: str, wallet_info: dict):
    ...
    try:
        ...
        if position_state in ("LONG", "SHORT"):
            ...
            if stop_hit(symbol, current_price):
                ...
                log_status(symbol, {
                    ...
                    "sr_ok_long": signal["sr_ok_long"],
                    "sr_ok_short": signal["sr_ok_short"],
                    "target_room_ok_long": signal["target_room_ok_long"],
                    "target_room_ok_short": signal["target_room_ok_short"],
                })
        ...
        if higher_trend == "UP" and signal["long_A"]:
            if not signal["sr_ok_long"]:
                block_reason = "SR_BLOCK"
            elif not signal["target_room_ok_long"]:
                block_reason = "TARGET_ROOM_BLOCK"
            else:
                long_ready = True
                entry_type = "A"

        elif higher_trend == "DOWN" and signal["short_A"]:
            if not signal["sr_ok_short"]:
                block_reason = "SR_BLOCK"
            elif not signal["target_room_ok_short"]:
                block_reason = "TARGET_ROOM_BLOCK"
            else:
                short_ready = True
                entry_type = "A"
        ...
        log_status(symbol, {
            ...
            "atr": signal["atr"],
            "volume_ratio": signal["volume_ratio"],
            "recent_resistance": signal["recent_resistance"],
            "recent_support": signal["recent_support"],
            "upside_room": signal["upside_room"],
            "downside_room": signal["downside_room"],
            "higher_tf_trend": higher_trend,
            "signal_state": signal_state,
            "position_state": "NONE",
            "pullback_active": state[symbol]["pullback_active"],
        ...
        if block_reason == "SR_BLOCK":
            print(f"[{now_kst_str()}] {symbol} NO TRADE | SR_BLOCK | up_room={signal['upside_room']:.4f} | down_room={signal['downside_room']:.4f}")
            return

        if block_reason == "TARGET_ROOM_BLOCK":
            print(f"[{now_kst_str()}] {symbol} NO TRADE | TARGET_ROOM_BLOCK | up_room={signal['upside_room']:.4f} | down_room={signal['downside_room']:.4f}")
            return
...
def main():
    ...
    print(f"[{now_kst_str()}] === Bybit Auto Bot v3 strategy start ===")
    bootstrap_state()
    ...
    while True:
        try:
            ...
            time.sleep(MAIN_LOOP_ERROR_SLEEP_SEC)
